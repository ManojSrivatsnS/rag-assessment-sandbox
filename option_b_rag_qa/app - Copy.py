...
def generate_answer(
    ...
    """
    Uses FAISS hits (chunks) for grounding, and optionally an LLM to synthesize
    a concise answer. If the top retrieval score is below `threshold`, abstains.

    - chunks: output of Retriever.search(...)  (each has: path, start, end, text, score)
    - llm: if True AND OPENAI_API_KEY present -> calls OpenAI Chat Completions
           else falls back to an extractive snippet from top chunk
    """
    top = chunks[0]["score"] if chunks else 0.0
    if (not chunks) or (top < threshold):
        return Answer(answer="I don't know.", citations=[], abstained=True)

    # format top chunks for prompting/citations
    def _fmt_chunks(chs: List[Dict[str, Any]]) -> str:
        lines = []
        for i, ch in enumerate(chs, 1):
            fname = os.path.basename(ch["path"])
    ...
    if llm and api_key:
        try:
            ...
            client = OpenAI(api_key=api_key,http_client=httpx.Client(timeout=30.0))  # no proxies)

            msgs = [
                {"role": "system", "content": (
                    "You are a careful assistant. Only answer using the provided sources. "
                    "If the sources are insufficient, say exactly: 'I don't know.' "
                    "Be concise, include short quotes, and cite sources like [1], [2]."
                )},
                {"role": "user", "content": (
            ...
            # if API fails, degrade gracefully to extractive answer
            print("LLM_ERROR:", e)   # <--- temporary debug
            text = chunks[0]["text"][:280] + " [1]"
    else:
        # LLM disabled or no key: simple extractive answer from top chunk
        text = chunks[0]["text"][:280] + " [1]"
...
def run_eval(index: str, qas_path: str, k: int, threshold: float,
             llm: bool = True, model: str | None = None,
             temp: float = 0.0, max_tokens: int = 350) -> Dict[str, float]:
    import os
    r = Retriever(index)
    lines = [ln for ln in open(qas_path, encoding="utf-8").read().splitlines() if ln.strip()]
    # Support JSONL or array (array should already be converted in main, but this is defensive)
    try:
...
def main():
    ...
    ap_eval.add_argument("--llm", choices=["on", "off"], default=None,
                         help="Force LLM on/off for eval. Default: on if OPENAI_API_KEY exists, else off.")
    ap_eval.add_argument("--model", type=str, default=None,
                         help="Override OPENAI_MODEL for eval.")
    ...
    if args.cmd == "ingest":
        ...
        if text.strip().startswith("["):
            ...
            qas_path = tmp  # use the temp JSONL file
